...
async function deleteEmployee(username) {
  ...
  // 1. מחק מכל הקולקציות ב-localStorage
  ...
  // 2. שמור localStorage ישירות — בלי לטריגר pushToFirebase אוטומטי
  _saveDBLocal(db);

  // 3. ודא שהמחיקה אכן נשמרה ב-localStorage
  const verify = JSON.parse(localStorage.getItem('vacSystem_v3') || '{}');
  if (verify.users && verify.users[username]) {
    console.error('[DELETE] localStorage עדיין מכיל את', username, '— ניסיון מחדש');
    delete verify.users[username];
    localStorage.setItem('vacSystem_v3', JSON.stringify(verify));
  }
  console.log('[DELETE] localStorage לאחר מחיקה — users:', Object.keys(JSON.parse(localStorage.getItem('vacSystem_v3') || '{}').users || {}));

  // 4. עצור את ה-listener לחלוטין לפני הדחיפה
  if (_fbUnsubscribe) { _fbUnsubscribe(); _fbUnsubscribe = null; }
  _isSyncing = true;
  _lastPushTime = Date.now();

  // 5. דחוף ל-Firebase ותחכה שהפעולה תסתיים
  try {
    await pushToFirebase(JSON.parse(localStorage.getItem('vacSystem_v3')));
