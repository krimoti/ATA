...
function _checkRL() {
	const k = _rlKey(),
		now = Date.now();
	if (!_loginAttempts[k]) _loginAttempts[k] = {
		count: 0,
		lock: 0
	};
	const r = _loginAttempts[k];
	if (r.lock > now) return {
		blocked: true,
		secs: Math.ceil((r.lock - now) / 1000)
	};
	return {
		blocked: false
	};
...
function _clearRL() {
	delete _loginAttempts[_rlKey()];
}
const deptElementIds = {
	'regDeptMulti': {
		dropdown: 'regDeptDropdown',
		tags: 'regDeptTags'
	},
	'newEmpDeptMulti': {
		dropdown: 'newEmpDeptDropdown',
		tags: 'newEmpDeptTags'
	},
};
...
function doLogin() {
	...
	const rl = _checkRL();
	if (rl.blocked) {
		showLoginError(`⛔ יותר מדי נסיונות. נסה בעוד ${rl.secs} שניות.`);
		return;
	}
	const db = getDB();
...
function regNextStep(step) {
	...
	if (step === 2) {
		...
		if (!fullName) {
			errEl.textContent = 'נא להזין שם מלא';
			errEl.style.display = 'block';
			return;
		}
		if (!username) {
			errEl.textContent = 'נא להזין שם משתמש';
			errEl.style.display = 'block';
			return;
		}
		if (!/^[a-zA-Z0-9_]+$/.test(username)) {
			errEl.textContent = 'שם משתמש: אנגלית/מספרים בלבד';
			errEl.style.display = 'block';
			return;
		}
		if (!dept) {
			errEl.textContent = 'נא לבחור מחלקה';
			errEl.style.display = 'block';
			return;
		}
		const db = getDB();
		if (db.users[username.toLowerCase()]) {
			errEl.textContent = 'שם משתמש זה כבר קיים';
			errEl.style.display = 'block';
			return;
		}
	}
...
function saveVacation(username, dateStr, type) {
	...
	if (db.approvalRequests) {
		db.approvalRequests.forEach(r => {
			if (r.username === username && r.year === year && r.month === month && (r.status === 'approved' || r.status === 'rejected')) {
...
function getQuota(username, year) {
	...
	if (!u || !u.quotas) return {
		annual: 0,
		initialBalance: 0,
		knownBalance: null,
		balanceDate: null
	};
	return u.quotas[String(year)] || {
		annual: 0,
		initialBalance: 0,
		knownBalance: null,
		balanceDate: null
...
function calcBalance(username, year) {
	...
	if (quota.balanceDate) {
		...
		knownBal = (quota.knownBalance !== null && quota.knownBalance !== undefined) ? quota.knownBalance : (quota.initialBalance || 0);
	} else {
...
function renderDashboard() {
	...
	if (currentUser.role === 'accountant' || currentUser.role === 'admin') {
		...
		document.getElementById('quotaUploadStatus').textContent = isJan ? ' — חלון הטעינה פתוח עד 09 לינואר!' : ' — החלון נסגר (01-09 לינואר בלבד)';
	} else {
...
function renderUpcoming() {
	...
	const upcoming = Object.entries(vacs).filter(([dt, type]) => {
		const d = new Date(dt);
...
function openDayModal(dateStr, hol, currentType, isSick) {
	...
	options.push({
		...
		label: '🔵 חצי יום חופשה',
		color: 'var(--info)',
		active: currentType === 'half'
	});
	if (!isHalfOnly) {
		options.push({
			type: 'wfh',
			label: '🟡 עבודה מהבית (WFH)',
			color: '#ca8a04',
			active: currentType === 'wfh'
		});
	}
	if (currentType) {
		options.push({
...
function exportPersonalReport() {
	...
	const sortedVacs = Object.entries(vacs).filter(([dt]) => dt.startsWith(String(year))).sort(([a], [b]) => a.localeCompare(b));
...
function renderAIForecast() {
	...
	const noVacEmp = employees.filter(u => {
		const total = Object.entries(db.vacations[u.username] || {}).filter(([dt, t]) => dt.startsWith(String(year)) && t !== 'wfh').reduce((s, [, t]) => s + (t === 'half' ? 0.5 : 1), 0);
		return total === 0;
	});
	...
	if (chartEl) {
		...
		if (!depts.length) {
			chartEl.innerHTML = '<p style="color:var(--text-muted);font-size:13px;">אין נתונים</p>';
			return;
		}
		...
		MONTHS.forEach(m => {
			html += `<th style="padding:4px 6px;color:var(--text-secondary);font-weight:600;text-align:center;">${m.slice(0,3)}</th>`;
		});
		html += '</tr>';
...
function renderManagerDashboard() {
	...
	if (cardsEl) cardsEl.innerHTML = [{
		icon: '😎',
		val: todayVacations.length,
		label: 'בחופשה היום',
		color: 'var(--primary)',
		bg: 'var(--primary-light)',
		names: todayVacations
	}, {
		icon: '🏠',
		val: todayWFH.length,
		label: 'עובדים מהבית',
	...
	// PENDING APPROVALS
	const pendingReqs = isAdmin ? (db.approvalRequests || []).filter(r => r.status === 'pending') : (db.approvalRequests || []).filter(r => r.status === 'pending' && r.assignedManager === myUsername);
	...
	if (upcomingEl) {
		...
		scopedUsers.forEach(user => {
			Object.entries(db.vacations[user.username] || {}).forEach(([dt, type]) => {
				if (dt >= todayStr && dt <= in30) upcoming.push({
					name: user.fullName,
					dept: getUserDept(user),
					date: dt,
					type
				});
			});
	...
	Object.values(db.users).forEach(user => {
		...
		Object.entries(db.vacations[user.username] || {}).forEach(([dt, type]) => {
			if (dt >= todayStr && dt <= in30 && type !== 'wfh') {
				const key = dt + '|' + dept;
				if (!byDateDept[key]) byDateDept[key] = {
					date: dt,
					dept,
					names: []
				};
...
function renderAuditLog() {
	...
	if (!log.length) {
		el.innerHTML = '<p style="color:var(--text-muted);font-size:13px;">אין רשומות</p>';
		return;
	}
	const icons = {
		login: '🔑',
...
function exportPayroll() {
	...
	Object.values(db.users).filter(u => u.role === 'employee').forEach(user => {
		...
		if (fullDays + halfDays > 0) rows.push({
			id: user.username,
			name: user.fullName,
			dept: getUserDept(user) || '',
			fullDays,
...
function approveRequest(reqId) {
	...
	(req.dates || []).forEach(dt => {
		db.vacations[req.username][dt] = req.type || 'full';
	});
...
function rejectRequestPrompt(reqId) {
	...
	if (!req) return;
	req.status = 'rejected';
	req.rejectedBy = currentUser.username;
...
function renderAdmin() {
	...
	if (empSel) {
		...
		empSel.innerHTML = '<option value="">— בחר עובד לעריכה —</option>' + sortedUsers.map(u => `<option value="${u.username}">${u.fullName} · ${u.dept||''}</option>`).join('');
	...
	filterEmpSel.innerHTML = '<option value="">כל העובדים</option>' + Object.values(db.users).map(u => `<option value="${u.username}">${u.fullName}</option>`).join('');
	// Department filter
	const filterDeptSel = document.getElementById('adminFilterDept');
	const allDepts = db.departments || [];
	filterDeptSel.innerHTML = '<option value="">כל המחלקות</option>' + allDepts.map(d => `<option value="${d}">${d}</option>`).join('');
...
function saveQuota() {
	...
	saveDB(db);
	const monthNames = ['ינואר', 'פברואר', 'מרץ', 'אפריל', 'מאי', 'יוני', 'יולי', 'אוגוסט', 'ספטמבר', 'אוקטובר', 'נובמבר', 'דצמבר'];
	const dateLabel = balanceDateObj ? `${balanceDateObj.getDate()} ${monthNames[balanceDateObj.getMonth()]} ${balanceDateObj.getFullYear()}` : `01 ינואר ${year}`;
	auditLog('quota_saved', `מכסה עודכנה ל-${db.users[emp].fullName}: ${annual} ימים`);
	showToast(`✅ מכסה נשמרה: ${annual} ימים | יתרת פתיחה (01/01): ${carryOver.toFixed(2)}`, 'success');
	renderAdmin();
}
...
function saveNewEmployee() {
	...
	const db = getDB();
	if (db.users[username]) {
		errEl.textContent = 'שם משתמש כבר קיים';
		errEl.style.display = 'block';
		return;
	}
	_sha256(pass).then(sha256hash => {
...
function getManagerScope(managerUsername, db) {
	...
	if (node && node.directDepts && node.directDepts.length) {
		...
		directDepts = Object.entries(deptManagers).filter(([, mgr]) => mgr === managerUsername).map(([dept]) => dept);
	}
	...
	const subManagers = Object.entries(orgTree).filter(([, n]) => n.parentManager === managerUsername).map(([u]) => u);
	...
	while (queue.length) {
		...
		Object.entries(orgTree).filter(([, n]) => n.parentManager === sub).forEach(([u]) => {
...
function renderDeptTags(containerId) {
	...
	if (selected.size === 0) {
		...
		tagsEl.innerHTML = [...selected].map(d => `<span class="dept-tag">${d} <span class="dept-tag-remove" onclick="removeDeptTag('${containerId}','${d}',event)">×</span></span>`).join('');
	}
}
...
function populateOrgTreeSelects() {
	const db = getDB();
	const allUsers = Object.values(db.users).filter(u => isUserActive(u) && u.role !== 'admin').sort((a, b) => a.fullName.localeCompare(b.fullName, 'he'));
	...
	if (deptBox) {
		...
		deptBox.innerHTML = depts.length ? depts.map(d => `<label class="org-dept-chip" id="orgChip_${d.replace(/[^a-zA-Zא-ת]/g,'_')}">
            <input type="checkbox" value="${d}"> ${d}
          </label>`).join('') : '<span style="color:var(--text-muted);font-size:12px;">אין מחלקות מוגדרות</span>';
...
function prefillOrgTreeForm() {
	...
	const directDepts = node.directDepts || Object.entries(deptManagers).filter(([, mgr]) => mgr === username).map(([d]) => d);
...
function saveOrgTreeEntry() {
	...
	const directDepts = [...document.querySelectorAll('#orgTreeDeptsCheckboxes input:checked')].map(cb => cb.value);
	const db = getDB();
...
function renderOrgTree() {
	...
	const nodes = {};
	Object.entries(orgTree).forEach(([u, n]) => {
		nodes[u] = { ...n
		};
	});
	Object.entries(deptManagers).forEach(([dept, mgr]) => {
		if (!nodes[mgr]) nodes[mgr] = {
			directDepts: [],
			parentManager: null
		};
	...
	function renderNode(username, depth) {
		...
		const scopeLabel = scope === null ? '👁️ כל העובדים' : scope.size === 0 ? '👁️ עצמו בלבד' : `👁️ ${scope.size} מחלקות`;
		const color = LEVEL_COLORS[Math.min(depth, LEVEL_COLORS.length - 1)];
		const kids = children[username] || [];
		const borderStyle = depth === 0 ? `border-right: 4px solid ${color};` : `border-right: 3px solid ${color};`;
		const marginRight = depth * 20;
		const deptsHtml = depts.length ? depts.map(d => `<span style="background:${color}18;color:${color};border:1px solid ${color}44;border-radius:20px;padding:2px 10px;font-size:11px;font-weight:700;">${d}</span>`).join('') : '';
		const kidsHtml = kids.length ? `<div style="margin-top:8px;padding-top:8px;border-top:1px dashed var(--border);">${kids.map(k => renderNode(k, depth+1)).join('')}</div>` : '';
...
function editOrgNode(username) {
	...
	setTimeout(() => {
		const mgrSel = document.getElementById('orgTreeManager');
		if (mgrSel) {
			mgrSel.value = username;
			prefillOrgTreeForm();
		}
		document.getElementById('orgTreeManager') ? .scrollIntoView({
			behavior: 'smooth',
			block: 'center'
		});
	}, 100);
...
function renderDeptManagerTable() {
	...
	const managerOptions = allUsers.map(u => `<option value="${u.username}">${u.fullName}${u.role==='admin'?' (אדמין)':u.role==='manager'?' (מנהל)':''}</option>`).join('');
	if (!depts.length) {
...
function addDepartment() {
	...
	const name = input.value.trim();
	if (!name) {
		showToast('⚠️ נא להזין שם מחלקה', 'warning');
		return;
	}
	const db = getDB();
	if (!db.departments) db.departments = [];
	if (db.departments.includes(name)) {
		showToast('⚠️ מחלקה זו כבר קיימת', 'warning');
		return;
	}
	db.departments.push(name);
	saveDB(db);
...
function renderVacationForecast() {
	...
	if (projectedEndBalance > 5) {
		statusEmoji = '🟢';
		statusText = 'קצב מצוין!';
		statusColor = 'var(--success)';
	} else if (projectedEndBalance >= 0) {
		statusEmoji = '🟡';
		statusText = 'בסדר גמור';
		statusColor = 'var(--warning)';
	} else {
		statusEmoji = '🔴';
		statusText = 'חריגה צפויה!';
...
async function changeAdminPassword() {
	...
	if (!adminUser) {
		showToast('⚠️ משתמש admin לא נמצא', 'warning');
		return;
	}
...
function updateApprovalStatusBadge() {
	...
	const db = getDB();
	const req = (db.approvalRequests || []).findLast ? (db.approvalRequests || []).findLast(r => r.username === currentUser.username && r.year === year && r.month === month) : [...(db.approvalRequests || [])].reverse().find(r => r.username === currentUser.username && r.year === year && r.month === month);
	if (!req) {
...
const PERMISSION_SECTIONS = [{
	...
	key: 'timeClockExportSection',
	label: '⏱️ ייצוא דיווחי שעות',
	adminOnly: false
}, {
	key: 'pendingRegistrationsSection',
	label: '👥 אישור הרשמות',
	adminOnly: false
}, {
	key: 'adminPasswordSection',
	label: '🔐 שינוי סיסמת ADMIN',
	adminOnly: true
}, {
	key: 'dataResetSection',
	label: '⚠️ איפוס נתונים',
	adminOnly: true
}, {
	key: 'deptMgmtSection',
	label: '🏢 ניהול מחלקות ומנהלים',
	adminOnly: false
}, {
	key: 'employeeListSection',
	label: '👥 רשימת עובדים',
	adminOnly: false
}, {
	key: 'allVacationsSection',
	label: '📋 כל בקשות החופשה',
	adminOnly: false
}, {
	key: 'approvalRequestsSection',
	label: '📨 בקשות אישור',
	adminOnly: false
}, {
	key: 'permissionsSection',
	label: '🔒 ניהול הרשאות',
	adminOnly: true
}, {
	key: 'canSendAnnouncements',
	label: '📢 שליחת הודעות לכל הצוות',
	adminOnly: false,
	special: true
}, ];
...
function renderPermissionsTable() {
	...
	const users = Object.values(db.users).filter(u => u.role !== 'admin' && isUserActive(u)).sort((a, b) => a.fullName.localeCompare(b.fullName));
	if (!users.length) {
...
function quickGrantPermission() {
	...
	const presets = {
		reports: {
			allVacationsSection: true,
			approvalRequestsSection: true
		},
		timeclock: {
			timeClockExportSection: true
		},
		employees: {
			employeeListSection: true,
			quotaUploadAdminSection: true,
			pendingRegistrationsSection: true
		},
		full: {
			timeClockExportSection: true,
			pendingRegistrationsSection: true,
			deptMgmtSection: true,
			quotaUploadAdminSection: true,
			employeeListSection: true,
			allVacationsSection: true,
			approvalRequestsSection: true
...
function populateCeoDashboard() {
	...
	depts.forEach(dept => {
		...
		if (pct >= 30) riskLines.push({
			dept,
			pct,
			size: deptUsers.length
		});
	});
	...
	if (riskLines.length) {
		...
		document.getElementById('ceoRiskText').textContent = `⚠️ ${top.dept}: ${top.pct}% מהצוות (${top.size} עובדים) בחופש/WFH שבוע הבא\n💡 המלצה: בדוק פרויקטים קריטיים ושקול דחיית אישורים` + (riskLines.length > 1 ? `\n+ ${riskLines.length-1} מחלקות נוספות בסיכון בינוני` : '');
	} else {
	...
	document.getElementById('ceoCostText').textContent = `📅 חופשות: ${vacDays} ימים — עלות ₪${Math.round(vacCost).toLocaleString()}\n🏠 WFH: ${wfhDays} ימים — חיסכון משוער ₪${wfhSave.toLocaleString()}\n💡 מבוסס על שכר יומי אישי לכל עובד`;
	// ===== CARD 4: BURNOUT =====
	...
	if (burnoutRisk.length) {
		...
		document.getElementById('ceoBurnoutText').textContent = `🚨 ${burnoutRisk.length} עובדים ללא חופשה ב-90 יום האחרונים:\n${names}${burnoutRisk.length>3?' ועוד...':''}\n💡 שקול שיחה אישית או חופשה כפויה`;
	} else {
...
function openCeoDayView() {
	...
	allUsers.forEach(u => {
		...
		if (t === 'full' || t === 'half') off.push({ ...u,
			dept
		});
		else if (t === 'wfh') wfh.push({ ...u,
			dept
		});
		else present.push({ ...u,
			dept
	...
	document.getElementById('ceoDayBody').innerHTML = section('בחופשה', '🏖️', 'var(--danger)', off) + section('עבודה מהבית', '🏠', 'var(--primary)', wfh) + section('במשרד', '✅', 'var(--success)', present) || '<div style="color:var(--text-muted);text-align:center;padding:24px;">אין נתונים להיום</div>';
...
function openCeoMessage() {
	if (!canSendAnnouncement()) {
		showToast('⛔ אין הרשאה לשלוח הודעות', 'error');
		return;
	}
...
function sendCeoMessage() {
	...
	if (!text) {
		showToast('נא לכתוב הודעה', 'warning');
		return;
	}
	const db = getDB();
...
function checkHandoverNeeded() {
	...
	if (msgEl && pending.firstDate) {
		const dateHeb = new Date(pending.firstDate + 'T00:00:00').toLocaleDateString('he-IL', {
			weekday: 'long',
			day: 'numeric',
			month: 'long'
		});
...
function saveHandover() {
	...
	const dateHeb = new Date(firstDate + 'T00:00:00').toLocaleDateString('he-IL', {
		weekday: 'long',
		day: 'numeric',
		month: 'long',
		year: 'numeric'
	...
	db.handoversArchive[key] = { ...record,
...
function checkPendingHandovers() {
	...
	const mine = Object.values(handovers).filter(h => h.managerUsername === currentUser.username && !h.seenByManager);
	...
	let html = '';
	mine.forEach(h => {
		const dateHeb = new Date(h.date).toLocaleDateString('he-IL', {
			weekday: 'long',
			day: 'numeric',
			month: 'long'
		});
...
function showHandoverNotificationModal(html, count) {
	...
	document.getElementById('handoverNotifBody').innerHTML = `<div style="font-size:13px;color:var(--text-muted);margin-bottom:14px;">יש לך ${count} פרוטוקול${count>1?'ות':''} ממתין${count>1?'ים':''} לעיון:</div>` + html;
...
function reportSickToday() {
	...
	if (db.sick[key]) {
		showToast('כבר דיווחת מחלה היום', 'info');
		return;
	}
	db.sick[key] = {
		username: currentUser.username,
...
function getEmployeeStatusToday(username) {
	...
	if (db.sick && db.sick[`${username}_${today}`]) {
		...
		return {
			status: 'sick',
			label: s.type === 'half' ? '🤒 חצי יום מחלה' : '🤒 חולה היום',
			color: '#ef4444',
			bg: '#fef2f2'
		};
	}
...
function renderWhereIsResults() {
	...
	const db = getDB();
	const users = Object.values(db.users).filter(u => u.role !== 'admin' && isUserActive(u) && (u.fullName.toLowerCase().includes(query) || u.username.toLowerCase().includes(query) || u.fullName.toLowerCase().split(' ').some(part => part.includes(query)))).sort((a, b) => a.fullName.localeCompare(b.fullName));
	if (!users.length) {
...
function openCeoAnnouncementsModal() {
	if (!canSendAnnouncement()) {
		showToast('⛔ אין הרשאה לצפות בהודעות', 'error');
		return;
	}
	const db = getDB();
	const allUsers = Object.values(db.users).filter(u => isUserActive(u) && u.role !== 'admin');
...
function deleteAnnouncement(id) {
	if (!canSendAnnouncement()) {
		showToast('⛔ אין הרשאה למחוק הודעות', 'error');
		return;
	}
...
async function parseBulkFile(file) {
	...
	reader.onload = e => {
		try {
			...
			if (rows.length < 2) {
				showToast('⚠️ הקובץ ריק או לא תקין', 'warning');
				return;
			}
			const db = getDB();
			const depts = db.departments || [];
			...
			rows.slice(1).forEach((row, i) => {
				...
				const existingUser = Object.values(db.users).find(u => u.fullName === `${firstName} ${lastName}`);
				let username;
				let isUpdate = false;
				if (existingUser) {
			...
			summaryEl.innerHTML = `<span style="color:var(--success);">✅ ${newCount} עובדים חדשים</span>` + (updateCount ? `<span style="color:var(--primary);margin-right:12px;">🔄 ${updateCount} יעודכנו</span>` : '') + (errors.length ? `<span style="color:var(--danger);margin-right:12px;">⚠️ ${errors.length} שגיאות</span>` : '');
			const year = new Date().getFullYear();
			tableEl.innerHTML = `
...
function applyBulkImport() {
	...
	const tempPass = (tpField && tpField.value.trim()) ? tpField.value.trim() : (getSettings().tempPassword || 'Welcome1');
	...
	_bulkImportData.forEach(p => {
		if (p.isUpdate) {
			...
			if (!u.quotas[year]) u.quotas[year] = {
				annual: 0,
				initialBalance: 0
			};
...
function deleteImportedEmployees() {
	...
	const toDelete = Object.values(users).filter(u => u.username !== 'admin' && u.role !== 'accountant' && u.role !== 'ceo');
	if (!toDelete.length) {
	...
	toDelete.forEach(u => {
		...
		if (db.sick) {
			db.sick = Object.fromEntries(Object.entries(db.sick).filter(([, s]) => s.username !== u.username));
		}
	...
	const remainingDepts = new Set(Object.values(db.users).map(u => getUserDept(u)).filter(Boolean));
	...
	showToast(`✅ נמחקו ${toDelete.length} עובדים` + (deptsRemoved ? ` + ${deptsRemoved} מחלקות ריקות` : ''), 'success', 5000);
	renderAdmin();
}
...
function saveTempPassword() {
	...
	if (!val) {
		showToast('⚠️ הזן סיסמה זמנית', 'warning');
		return;
	}
	const db = getDB();
	if (!db.settings) db.settings = {};
...
function renderAIStaffingForecast() {
	...
	const myManagedDepts = isAdmin ? null : Object.entries(db.deptManagers || {}).filter(([, mgr]) => mgr === currentUser.username).map(([dept]) => dept);
	...
	for (let w = 0; w < 8; w++) {
		...
		we.setDate(ws.getDate() + 4); // Friday
		const dates = [];
		for (let d = 0; d < 5; d++) {
      ...
      <div style="overflow-x:auto;-webkit-overflow-scrolling:touch;">
        <table style="border-collapse:collapse;min-width:500px;width:100%;font-size:12px;">
          ...
          <tbody>
            ...
            `).join('')
} < /tbody> < /table > < /div> < div style = "display:flex;gap:12px;margin-top:8px;font-size:11px;color:var(--text-muted);" > < span > 🟥70 % + < /span > < span > 🟨40 - 70 % < /span > < span > 🟩10 - 40 % < /span > < span > ⬜נקי < /span > < /div > < /div>` : ''}
`;
}
...
function renderEmployeeScores() {
  ...
  el.innerHTML = ` < div style = "display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px;" > < div class = "section-title"
style = "margin-bottom:0;" > < span class = "section-title-icon" > 🏆 < /span> AI — ציוני רווחת עובדים</div > < span style = "font-size:11px;background:var(--primary-light);color:var(--primary);padding:4px 10px;border-radius:12px;font-weight:700;" > מסודר לפי סיכון < /span> < /div > < div style = "display:flex;flex-direction:column;gap:8px;" > $ {
	scores.map(({
		u,
		wellbeing,
		burnout,
...
function saveTCRecord() {
	...
	const status = document.getElementById('tcSaveStatus');
	if (!date) {
		status.innerHTML = '<span style="color:var(--danger);">⚠️ בחר תאריך</span>';
		return;
	}
	if (!entry) {
		status.innerHTML = '<span style="color:var(--danger);">⚠️ הזן שעת כניסה</span>';
		return;
	}
	if (!exit) {
		status.innerHTML = '<span style="color:var(--danger);">⚠️ הזן שעת יציאה</span>';
		return;
	}
	// Calc total
	const [eh, em] = entry.split(':').map(Number);
...
function exportTimeClock(range) {
	...
	if (range === 'today') {
		...
		if (!from || !to) {
			showToast('⚠️ בחר טווח תאריכים', 'warning');
			return;
		}
		if (from > to) {
			showToast('⚠️ תאריך התחלה גדול מהסיום', 'warning');
			return;
		}
	}
	const db = getDB();
	...
	if (!rows.length) {
		showToast('⚠️ אין דיווחים בטווח זה', 'warning');
		return;
	}
	...
	const csvRows = rows.map(r => [r.fullName, r.dept || '', r.date, r.entry || '', r.exit || '', r.total || '', (r.note || '').replace(/,/g, '；')].map(v => `"${v}"`).join(','));
	const BOM = '\uFEFF';
...
function calcDaysSinceLastVacation(vacs) {
	const today = new Date();
	const vacDates = Object.entries(vacs).filter(([, t]) => t === 'full' || t === 'half').map(([dt]) => new Date(dt + 'T00:00:00')).filter(d => d <= today).sort((a, b) => b - a);
...
async function forgotStep1Next() {
	...
	const user = db.users[username];
	if (!user) {
		errEl.textContent = 'שם משתמש לא נמצא במערכת';
		errEl.style.display = 'block';
		return;
	}
	if (!user.email) {
	...
	if (btn) {
		btn.textContent = 'שלח מייל איפוס 📧';
		btn.disabled = false;
	}
	if (result.success) {
...
function forgotSetNewPassword() {
	...
	errEl.style.display = 'none';
	if (!p1 || p1.length < 4) {
		errEl.textContent = 'סיסמה חייבת להיות לפחות 4 תווים';
		errEl.style.display = 'block';
		return;
	}
	if (p1 !== p2) {
...
function doChangePassword() {
	...
	const user = db.users[currentUser.username];
	if (newPass.length < 4) {
		errEl.textContent = 'הסיסמה החדשה חייבת להיות לפחות 4 תווים';
		errEl.style.display = 'block';
		return;
	}
	if (newPass !== newPass2) {
...
function startRealtimeListener() {
	...
	_fbUnsubscribe = firebaseDB.collection('vacationSystem').doc('data').onSnapshot(doc => {
		if (!doc.exists) return;
		const data = doc.data();
		// Don't apply if this was our own write (updatedBy = us, within 2s)
		const updatedBy = data.updatedBy;
		const updatedAt = data.updatedAt ? new Date(data.updatedAt) : null;
		const isOwnWrite = updatedBy === currentUser ? .username && updatedAt && (Date.now() - updatedAt.getTime()) < 2000;
...
async function connectFirebase() {
	...
	await initFirebase();
	if (btn) {
		btn.textContent = '🔥 התחבר ל-Firebase';
		btn.disabled = false;
	}
	openFirebaseModal();
}
...
function disconnectFirebase() {
	...
	document.getElementById('fbDisconnectBtn').style.display = 'none';
	document.getElementById('fbResetBtn').style.display = 'none';
	document.getElementById('firebaseStatusDiv').innerHTML = '<div class="alert alert-info"><span class="alert-icon">🔌</span><span>נותק. הנתונים נשמרים מקומית בלבד.</span></div>';
	showToast('🔌 נותק מ-Firebase', 'warning');
}
async function resetFirebase() {
	if (!firebaseConnected || !firebaseDB) {
		showToast('⚠️ לא מחובר ל-Firebase', 'warning');
		return;
	}
...
function doSubmitRequest(note) {
	...
	db.approvalRequests = db.approvalRequests.filter(r => !(r.username === currentUser.username && r.year === year && r.month === month && r.status === 'pending'));
	...
	reader.onload = function(e) {
		try {
			...
			if (!data || typeof data !== 'object' || !data.users) {
				showToast('❌ קובץ לא תקין — חסר מידע עובדים', 'error');
				return;
			}
			const curDB = getDB();
...
window.addEventListener('load', function() {
	...
	try {
		...
		if (raw) {
			...
			// Company name
			const company = (db.settings ? .companyName && db.settings.companyName !== 'החברה שלי') ? db.settings.companyName : '';
...
async function sendAIMessage() {
	...
	if (!msg) return;
	if (!currentUser) {
		showToast('⚠️ יש להתחבר כדי להשתמש ב-AI', 'warning');
		return;
	}
	input.value = '';
...
function appendAIMessage(text, role) {
	...
	const safe = text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\n/g, '<br>');
...
function renderHandoverList() {
	...
	const myDepts = (() => {
		const deptManagers = db.deptManagers || {};
		return Object.entries(deptManagers).filter(([dept, mgr]) => mgr === currentUser.username).map(([dept]) => dept);
	})();
	...
	if (badge) {
		...
		if (newCount > 0) {
			badge.textContent = newCount + ' חדש';
			badge.style.display = 'inline';
		} else badge.style.display = 'none';
	}
	el.innerHTML = list.map(h => {
		const dateHeb = new Date(h.date + 'T00:00:00').toLocaleDateString('he-IL', {
			weekday: 'long',
			day: 'numeric',
			month: 'long'
		});
...
function printHandoverPDF(key) {
	...
	if (!h) {
		showToast('פרוטוקול לא נמצא', 'error');
		return;
	}
	const dateHeb = new Date(h.date + 'T00:00:00').toLocaleDateString('he-IL', {
		weekday: 'long',
	...
	const createdHeb = h.createdAt ? new Date(h.createdAt).toLocaleDateString('he-IL', {
		day: 'numeric',
		month: 'long',
		year: 'numeric',
		hour: '2-digit',
	...
	const tasksHtml = h.tasks.map((t, i) => `<tr><td style="padding:8px 12px;border-bottom:1px solid #eee;color:#666;width:28px;">${i+1}.</td><td style="padding:8px 12px;border-bottom:1px solid #eee;">${t}</td></tr>`).join('');
...
function renderMyHandoverCard() {
	...
	const seen = new Set();
	const myRecords = [...Object.values(db.handoversArchive || {}), ...Object.values(db.handovers || {})].filter(h => h.user === currentUser.username).filter(h => {
...
function openMyHandoverPopup() {
	const db = getDB();
	const today = new Date().toISOString().split('T')[0];
	const allSources = [...Object.values(db.handoversArchive || {}), ...Object.values(db.handovers || {})];
	const seen2 = new Set();
	const myRecords = allSources.filter(h => h.user === currentUser.username).filter(h => {
		const k = h.user + '_' + h.date;
  ...
  ` + myRecords.map(h => {
		...
		const fmtDate = dt => new Date(dt + 'T00:00:00').toLocaleDateString('he-IL', {
			day: 'numeric',
			month: 'long',
			year: 'numeric'
		});
		const fmtDateShort = dt => new Date(dt + 'T00:00:00').toLocaleDateString('he-IL', {
			day: 'numeric',
			month: 'long'
		});
		...
		const tasks = h.tasks.map((t, i) => `<div style="display:flex;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
        <span style="font-size:12px;color:var(--text-muted);min-width:18px;">${i+1}.</span>
        <span style="font-size:13px;color:var(--text);">${t}</span>
      </div>`).join('');
		const contact = h.contact ? `<div style="display:flex;align-items:center;gap:10px;margin-top:12px;padding:10px 14px;background:var(--primary-light);border-radius:10px;">
...
function exportHandoversExcel() {
	...
	const today = new Date().toISOString().split('T')[0];
	const isAdmin = currentUser.role === 'admin';
	const list = Object.values(handovers).filter(h => (isAdmin || h.managerUsername === currentUser.username) && h.date >= today).sort((a, b) => a.date.localeCompare(b.date));
	if (!list.length) {
		showToast('אין פרוטוקולים לייצוא', 'warning');
		return;
	}
...
function saveCustomQAEntry() {
	...
	if (!questionRaw || !answer) {
		showToast('⚠️ נא למלא שאלה ותשובה', 'warning');
		return;
	}
...
function exportCustomQA() {
	...
	const qa = db.customQA || [];
	if (!qa.length) {
		showToast('אין שאלות לייצוא', 'warning');
		return;
	}
	const BOM = '﻿';
	const header = 'שאלה,ניסוחים נוספים,תשובה,תגיות';
	const rows = qa.map(e => ['"' + (e.question || '').replace(/"/g, '""') + '"', '"' + (e.aliases || []).join('|').replace(/"/g, '""') + '"', '"' + (e.answer || '').replace(/"/g, '""') + '"', '"' + (e.tags || '').replace(/"/g, '""') + '"'].join(','));
