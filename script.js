...
async function deleteEmployee(username) {
  ...
  // 3. דחוף ל-Firebase ותחכה שהפעולה תסתיים לפני כל המשך
  try {
    await pushToFirebase();
  } catch(e) {
    console.warn('deleteEmployee push error:', e);
  }

  // 4. רק אחרי push מוצלח — תעד ורנדר
  if (!db.auditLog) db.auditLog = [];
  db.auditLog.unshift({ ts: new Date().toISOString(), user: currentUser?.username || 'admin', action: 'employee_deleted', details: `עובד נמחק: ${name}` });
  if (db.auditLog.length > 500) db.auditLog = db.auditLog.slice(0, 500);
  _saveDBLocal(db);
...
async function pushToFirebase() {
  ...
  try {
    ...
    // Keep suppressing listener for 3s after write completes (Firebase snapshot delay)
