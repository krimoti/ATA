// ============================================================
// DAZURA AI ENGINE v5.0 — DAZY Edition
// Built by מוטי קריחלי 🏆
// Smart • Warm • Context-aware • Bilingual
// ============================================================

const DazuraAI = (() => {

  let conversationHistory = [];
  const MAX_HISTORY = 20; // 10 סבבים = 20 הודעות
  ...
  const MOTI_CREATOR = 'מוטי קריחלי בנה אותי מ-0 🏆 — בלי מוטי לא היה DAZY.';
...
דוגמה: שינוי מ-20 ל-24 ימים ב-1.7 → מוסיף ~2 ימים (חצי שנה).`,
  ...
  function respondThanks(user) {
    const n = user.fullName.split(' ')[0];
    const K = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
    if (K && K.THANKS_RESPONSES) {
      const pool = K.THANKS_RESPONSES;
...
`**📡 שאלות שאני עונה עליהן:**\n• "מי מהצוות בחופשה / WFH היום? מחר? ביום שלישי?"\n• "מה סטטוס הבקשה שלי?"\n• "מה הימים המומלצים לחופש?"\n• "מה שם החברה?" / "גרסת המערכת"\n• "איך משנים סיסמה?" / "איך מעדכנים מייל?"\n\n💡 כתוב בחופשיות — אני מבין עברית טבעית`;
  ...
  function respondUnknown(rawInput, currentUser, db) {
    ...
    // ── 6. Default — שלושת האפשרויות: ניחוש + הודאה + ניסיון ─────
    const K3 = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
    const guess = K3 ? K3.guessIntent(rawInput) : '"מה אתה יכול?"';
    const today2 = dateToKey(new Date());
    const allStats2 = getStatsForDate(db, today2);
    const absent2 = allStats2.vacation.length + allStats2.sick.length;
    const pending2 = (db.approvalRequests||[]).filter(r=>r.status==='pending').length;

    // ניחוש — אולי התכוון ל...
    const guessLine = `אולי התכוונת ל: ${guess}?`;
    // הודאה + נתון אמיתי
    const dataLine = absent2 > 0
      ? `ואגב — היום ${absent2} עובדים נעדרים.`
      : (pending2 > 0 ? `ואגב — ${pending2} בקשות ממתינות לאישור.` : 'כולם במשרד היום 📍');
    // הצעה
    const tryLine = lang === 'en'
      ? 'Try: "who is on vacation?" / "my balance?" / "help"'
      : 'נסה: "מי בחופשה היום?" / "מה היתרה שלי?" / "עזרה"';

    return `${firstName}, לא בטוח שהבנתי 🤔
...
${tryLine}`;
  ...
  function respond(rawInput, currentUser, db) {
    ...
    const firstName = currentUser.fullName.split(' ')[0];
    const K = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
    const lang = K ? K.detectLanguage(rawInput) : 'he';
    const emotion = K ? K.detectEmotion(rawInput) : 'neutral';
    ...
    lastContext.turnCount = (lastContext.turnCount||0) + 1;
    lastContext.lang = lang;

    const t = rawInput.toLowerCase().trim();

    // ── DAZY: זיהוי שיחה חברתית לפני הכל ──
    if (K && K.isSocial(rawInput)) {
      const r = K.respondSocial(rawInput, firstName, lang);
      conversationHistory.push({role:'ai',text:r}); return r;
    }

    // ── DAZY creator ──
    if (/מי יצר אותך|מי בנה אותך|מי עשה אותך|מי פיתח אותך|who made you|who created you|who built you/.test(t)) {
      const r = K ? K.DAZY_PERSONALITY.creator(firstName) : MOTI_CREATOR;
      conversationHistory.push({role:'ai',text:r}); return r;
    }
    if (/מוטי.*תותח|תותח.*מוטי/.test(t)) {
      const r = 'כן! מוטי קריחלי בנה את DAZY מ-0 🏆 — עם דגש על חכמה, מהירות ונשמה ישראלית.';
      conversationHistory.push({role:'ai',text:r}); return r;
    }
    ...
    switch(intent) {
      ...
      case 'who_is_moti':
      case 'who_is_dazy': {
        const K2 = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        response = K2
          ? K2.DAZY_PERSONALITY.whoIsDazy(firstName, hasAdminAccess(currentUser), hasManagerAccess(currentUser))
          : respondWhoIsMoti(currentUser);
        break;
      }
      ...
      case 'who_vacation': {
        ...
        lastContext={...lastContext, intent, dateInfo, resultList:filtered, dept:null};
        const vacResp = dateInfo.range?respondWhoAtRange(db,dateInfo,currentUser,'vacation'):respondWhoAt(db,dateInfo,currentUser,'vacation');
        // הוסף flair אנושי
        const Kv2 = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        const flair = Kv2 && filtered.length ? Kv2.addVacationFlair(filtered, dateInfo.label) : null;
        response = flair ? vacResp + '\n\n_' + flair + '_' : vacResp;
        break;
      }
      case 'who_wfh': {
        const stats = getStatsForDate(db, dateToKey(dateInfo.date||new Date()));
...
• אשר בסבבים ולא הכל בבת אחת`; break;
      ...
      case 'dazy_creator': {
        const Kd = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        response = Kd ? Kd.DAZY_PERSONALITY.creator(firstName) : MOTI_CREATOR;
        break;
      }
      case 'dazy_laugh': {
        const Kl = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        response = Kl ? Kl.DAZY_PERSONALITY.laugh() : 'אין לי בדיחה עכשיו 😄';
        break;
      }
      case 'dazy_feel': {
        const Kf = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        response = Kf ? Kf.DAZY_PERSONALITY.feel() : 'מרגיש מצוין תודה!';
        break;
      }
      case 'dazy_compliment': {
        const Kc = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        response = Kc ? Kc.DAZY_PERSONALITY.compliment(firstName) : 'אתה/את מדהים/ה! 🌟';
        break;
      }
      case 'dazy_vs_human': {
        const Kv = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        response = Kv ? Kv.DAZY_PERSONALITY.vsHuman(firstName) : 'אני כאן לעזור, לא להתחרות 😊';
        break;
      }
      case 'off_topic': {
        const Ko = typeof DazuraKnowledge !== 'undefined' ? DazuraKnowledge : null;
        const isEn2 = lang === 'en';
        const topicMatch = rawInput.match(/מזג אוויר|weather|בישול|cooking|ספורט|sports|פוליטיקה|politics|bitcoin|crypto/i);
        const topic = topicMatch ? topicMatch[0] : rawInput.split(' ')[0];
        response = Ko ? Ko.DAZY_PERSONALITY.offTopic(topic, firstName, isEn2) : 'זה מחוץ לתחום שלי 😊 שאל/י על חופשות ונוכחות.';
        break;
      }
  ...
  function clearHistory() {
    ...
    lastContext = typeof DazuraKnowledge !== 'undefined'
      ...
      : { intent:null, dateInfo:null, resultList:[], subject:null, subjectName:null, dept:null, data:null, lastQuestion:null, turnCount:0, lang:'he' };
  }
