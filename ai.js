...
// ── שכבת אישיות גלובלית (מוסיפה חיים לכל תשובה) ──
const PERSONALITY = {
  warmStart: (name) => [
    `היי **${name}**! 👋 `,
    `בדיוק בשביל זה אני כאן, **${name}** 😊 `,
    `קיבלתי! `
  ],
  randomEmoji: () => ['🌴','🏠','📅','✅','🤍','😌'][Math.floor(Math.random()*6)]
};
  // ============================================================
  // DATE PARSER
  // ============================================================
  ...
  // ============================================================
  const INTENT_RULES = [
{ name:'who_vacation', score: t => /מי (ב|הוא|היא|נמצא|יצא|בחופשה|חופש|חופשה)/.test(t) && !/של|יתרה|ניצול/.test(t) ? 15 : 0 },
{ name:'emp_balance',  score: t => /(יתרה|ימים|חופשה) (של|של|ל)[^\s]+/.test(t) ? 14 : 0 },
...
`**📡 שאלות שאני עונה עליהן:**\n• "מי מהצוות בחופשה / WFH היום? מחר? ביום שלישי?"\n• "מה סטטוס הבקשה שלי?"\n• "מה הימים המומלצים לחופש?"\n• "מה שם החברה?" / "גרסת המערכת"\n• "איך משנים סיסמה?" / "איך מעדכנים מייל?"\n\n💡 כתוב בחופשיות — אני מבין עברית טבעית`;
  ...
  if (uname && hasManagerAccess(currentUser)) {
    return respondEmpBalance(db.users[uname], db, new Date().getFullYear());
  }
  ...
  return `לא הבנתי ב-100%, **${first}** ${PERSONALITY.randomEmoji()}\n\nנסה לנסח מחדש או תכתוב "עזרה" ואני אראה לך דוגמאות מדויקות לפי התפקיד שלך.`;
}
...
response = addPersonality(response, currentUser);
